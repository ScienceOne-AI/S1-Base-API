...
class PointTransformerAttention(nn.Module):
    ...
    def forward(self, q_feat, k_feat, q_pos, k_pos):
        """
        ...
        if self.k is None:
            ...
            attn = attn / (attn.sum(dim=-1, keepdim=True)+1e-6)   # (B, S, N)
            weighted_v = attn.unsqueeze(-1) * v.unsqueeze(1)      # (B, S, N, D)
            ...
            attn = q.unsqueeze(2) - k_neighbors + pos_enc         # (B, S, k, D)
            attn = self.softmax(attn.sum(-1))                     # (B, S, k)
...
class PointSlotAttention_upsample(nn.Module):
    def __init__(self, input_dim, dim, iters=3, k=None):
        super().__init__()
        self.iters = iters
        self.dim = dim
    ...
    def forward(self, inputs, pos, slots, slot_pos):
        """
        inputs: (B, N, D)  特征
        slots:  (B, K, D)  特征
        """
        B, N, D = slots.shape
        inputs = self.norm_inputs(self.input_proj(inputs))
